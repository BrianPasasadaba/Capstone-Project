    ...
    return cookieValue;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function filterTable(searchTerm) {
                ...
                rows.forEach(row => {
                    const firNumber = row.querySelector('td:nth-child(1)')?.textContent.toLowerCase() || ''; // FIR Number
                    const location = row.querySelector('td:nth-child(2)')?.textContent.toLowerCase() || ''; // Location
                    const date = row.querySelector('td:nth-child(3)')?.textContent.toLowerCase() || ''; // Date
        
                    // Show row if any column matches the search term
                    if (firNumber.includes(term) || location.includes(term) || date.includes(term)) {
            ...
            searchInput.addEventListener('input', function () {
                ...
                // Hide suggestions box for now (can be extended for autocomplete)
